...
export const ClaudeFileEditor: React.FC<ClaudeFileEditorProps> = ({
  ...
  const handleSave = async () => {
    try {
      ...
      setToast({ message: t("execution.fileSavedSuccess"), type: "success" });
    } catch (err) {
      console.error("Failed to save file:", err);
      setError("Failed to save CLAUDE.md file");
      setToast({ message: t("execution.fileSaveError"), type: "error" });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full">
        ...
        >
          ...
          >
            ...
            {saving ? t("execution.saving") : t("common.save")}
          </Button>
        </motion.div>
